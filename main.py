...
db.commit()
# Створення таблиці для записів адміністраторських прийомів
cursor.execute('''
...
db.commit()
# Токен вашого бота
bot = telebot.TeleBot('6873571266:AAEPwYDDxmwkuOQ3fF2n9A7YHdtx1fFkDyY')
...
def get_admin_appointments():
    # Створити нове з'єднання і курсор у тому самому потоці
    with sqlite3.connect('appointments.db') as db:
        cursor = db.cursor()
        cursor.execute("SELECT appointment_datetime FROM admin_appointments")
        appointments = cursor.fetchall()

    return [appointment[0] for appointment in appointments]

admin_appointments = get_admin_appointments()
...
def process_admin_input(message):
    ...
    with sqlite3.connect('appointments.db') as db:
        cursor = db.cursor()
        cursor.execute("INSERT INTO admin_appointments (appointment_datetime) VALUES (?)", (new_appointment,))
        db.commit()
        global admin_appointments
        admin_appointments = get_admin_appointments()
...
def make_appointment(message):
    global admin_appointments  # Забезпечте використання глобальної змінної

    admin_appointments = get_admin_appointments()  # Оновіть список admin_appointments
